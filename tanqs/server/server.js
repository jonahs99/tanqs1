...
var GameServer = function (http, config) {
  ...
  this.config = config;
  this.server_config = {
    ms_frame: this.ms_frame,
    frames_update: this.frames_update,
    game_type: "ctf",
  };
    ...
    if (tank.reserved && !tank.ai) {
      ...
      for (var j = 0; j < this.topten.length; j++) {
        ...
        if (tank.client.id == top.id) {
          var done = true;
          if (score > top.score) top.score = score;
          break;
        }
      }
      if (!done) {
        this.topten.push({
          name: tank.client.name,
          color: tank.color,
          score: score,
          id: tank.client.id,
        });
...
GameServer.prototype.add_client = function (socket, user_string) {
  var client = {
    ...
    name: "",
    state: "pre-login",
    stats: { kills: 0, deaths: 0, death_record: 0, points: 0 },
  };
...
GameServer.prototype.player_kill = function (killer_id, killed_id, bullet) {
  ...
  if (killer_tank.ai && killed_tank.ai) {
    ...
    var chat_msg =
      '&gt&gt <i> <span style="color:' +
      killer_tank.color +
      '">' +
      killer_tank.client.name +
      "</span> " +
      verb +
      ' <span style="color:' +
      killed_tank.color +
      '">' +
      killed_tank.client.name +
      "</span>. </i>";
    ...
    if (flag_type == "default") {
      point_award += 5;
      special_text += " (flagless kill!)";
    }
    if (killed_tank.flag_team > -1) {
      point_award += 10;
      special_text += " (carrier kill!)";
    }
    if (killer_tank.flag_team > -1) {
      point_award += 15;
      special_text += " (flag kill!)";
    }

    var kill_time = 1000; // ms for a double kill, x2 for triple
    if (
      killer_tank.kill_timer <
      (kill_time / this.ms_frame) * killer_tank.kill_count
    ) {
    ...
    killer_tank.client.stats.points += point_award;
    this.send_kill(
      killer_tank.client.socket,
      killed_id,
      "+" + point_award + special_text,
    );
  }
};
...
GameServer.prototype.flag_capture = function (tank_id, team, team_size) {
  ...
  var chat_msg =
    ...
    '">' +
    tank.client.name +
    '</span> CAPTURED the <span style="color:' +
    team_color +
    '">' +
    team_name +
    " flag</span>!</i>";
...
GameServer.prototype.send_who = function () {
  ...
  var msg = {
    time: {
      m: 20 - (d.getMinutes() % 20) - (d.getSeconds() == 0 ? 0 : 1),
      s: 60 - d.getSeconds(),
    },
    clients: [],
    teams: [],
...
GameServer.prototype.on_chat = function (socket, msg) {
  ...
  if (client.tank_id > -1) {
    ...
    this.send_chat(
      '<span style="color:' + color + '"><b>' + name + "</b></span>: " + text,
    );
  }
};
