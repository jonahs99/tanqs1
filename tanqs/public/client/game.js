...
Game.prototype.change_state = function (state) {
  if (state == GameState.DISCONNECTED) {
    ...
    splash_text.innerHTML = "tanqs.net";
    ...
    if (game.player_tank.killed_by > -1) {
      ...
      splash_text.innerHTML =
        ...
        killer.name +
        "</span> rekt you ;&gt";
      this.renderer.following = game.player_tank.killed_by;
    } else {
...
Game.prototype.update = function () {
  ...
  if (this.state == GameState.GAME) {
    if (this.player_tank.alive) {
      this.player_tank.steer_target.set_xy(
...
Game.prototype.begin_simulation = function () {
  game_loop_interval = setInterval(
    function () {
      this.update();
  ...
  send_input_interval = setInterval(
    function () {
